...
RP2040 firmware and file uploader:
  ./scriptloader.sh           - Upload files to RP2040
  ./scriptloader.sh --first   - Flash MicroPython firmware
  ./scriptloader.sh --wipe    - Wipe device and flash MicroPython
...
get_files() {
	if [[ "$FIRST_TIME_FLASH" == true ]]; then
		...
		find "$verdir" -name "*.py" -o -name "*.bin" | head -10
	fi
}
...
handle_wipe_flash() {
	...
	for file in $(find "src/$version" -name "*.py" -o -name "*.bin"); do
		[[ -f "$file" ]] || continue
		((total++))
		upload_python "$file" && ((success++))
	done

	echo "Files uploaded: $success/$total"
	[[ $success -eq $total ]]
}
