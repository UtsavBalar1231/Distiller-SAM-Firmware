    ...
    if missing:
        ...
        if 'ampy' in missing:
            ...
            try:
                ...
                print("Failed to install ampy. Please install manually: pip3 install adafruit-ampy")
                return False
        if 'mpy-cross' in missing:
            ...
            try:
                ...
                print("Failed to install mpy-cross. Please install manually: pip3 install mpy-cross")
                return False
...
def flash_uf2_file_applescript(uf2_filename, description):
    ...
    try:
        ...
        result = subprocess.run(['osascript', '-e', applescript_copy], 
...
def flash_uf2_file(uf2_filename, description):
    ...
    try:
        ...
        try:
            ...
            result = subprocess.run(['cp', uf2_path, dest_path], 
...
def main():
    ...
    parser.add_argument('--wipe', action='store_true', help='Wipe flash and install MicroPython')
    parser.add_argument('--first', action='store_true', help='First time flash (install MicroPython only)')
    parser.add_argument('--compile', action='store_true', help='Compile Python files to bytecode (.mpy) before upload')
