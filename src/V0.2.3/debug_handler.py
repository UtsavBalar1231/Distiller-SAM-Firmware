# Author: PamirAI - Centralized Debug Handler
# Date: 2025-07-15  
# Version: 1.0.0
# Description: Centralized debug system with runtime level control and minimal overhead
...
class DebugHandler:
    ...
    def _log_startup(self):
        ...
        self.log(self.LEVEL_INFO, self.CAT_SYSTEM, 
    ...
    def set_category_filter(self, category, enabled):
        ...
        if category in self.category_filters:
            ...
            if self.enable_uart_output and self.debug_level >= self.LEVEL_INFO:
                print(f"[{utime.ticks_ms()}] DEBUG: Category {category} {'enabled' if enabled else 'disabled'}")
    ...
    def get_statistics(self):
        ...
        if uptime > 0:
            stats["messages_per_second"] = (self.stats["total_messages"] * 1000) // uptime
        else:
    ...
    def send_debug_code(self, uart, category_code, debug_code, param=0):
        ...
        if packet and uart:
            try:
                uart.write(packet)
                self.log_verbose(self.CAT_UART, f"Debug code sent: cat={category_code}, code={debug_code}")
